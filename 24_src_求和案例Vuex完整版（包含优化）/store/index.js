...
import Vuex from 'vuex'; // 确保导入 Vuex

//使用vuex来集中管理状态，必要
//new store的前提是必须要使用Vuex插件

Vue.use(Vuex);//确保使用Vuex
...
const actions = {
    ...
    incrementIfOdd(context,value){
        ...
        if(context.state.sum % 2){
            console.log('@')
            context.commit('INCREMENT',value);
            //context.state.sum += 1;//这样可以实现但是记住本次对状态的改变开发者工具将无法捕获，因为开发者工具是对mutations对话
        }
    },
    ...
    //},
    //demo2(context,value){
    //     console.log('在acton的demo中完成最终的逻辑')；
    //     if(context.state.sum % 2){
    //         console.log(‘@’)
    //         context.commit('INCREMENT',value);
    //   }
